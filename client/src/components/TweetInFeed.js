...
import { MessageCircle, Calendar, Matic, Bin } from '@web3uikit/icons'
...
const TweetInFeed = props => {
  ...
  }, [reloadComponent]) // doenst load if reloadComponent doesn't change
  ...
  if (loadingState == 'loaded' && !tweets.length)
    return <h1 className='loading'>No Tweet availabe</h1>
  return (
    <>
      {tweets.map((tweet, i) => (
        <div className='feedTweet'>
          ...
          <div className='completeTweet'>
            <div className='who'>
              {tweet.userName} <div className='accWhen'>{tweet.tweeter} </div>
            </div>
